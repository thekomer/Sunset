...
def __init__(self, path, force=False, filter=None):
	...
	def startElement(self, name, attrs):
		if self.inside[-1] == self.ROOT:
			...
			if (name == "g" and ('inkscape:groupmode' in attrs) and ('inkscape:label' in attrs) and attrs['inkscape:groupmode'] == 'layer' and attrs['inkscape:label'].startswith('Baseplate')):
